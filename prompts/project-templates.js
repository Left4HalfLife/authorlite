 ...
 */
export function deriveDependencies(steps) {
    if (!Array.isArray(steps) || steps.length === 0)
        return;
    const isChapterWrite = (s) => (s.skill === 'write' || s.phase === 'writing') && typeof s.chapterNumber === 'number';
    const isChapterReview = (s) => {
        if (typeof s.chapterNumber !== 'number' || isChapterWrite(s))
            return false;
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        let deps = prev ? [prev.id] : [];
        if (isChapterWrite(s)) {
            const prevWrite = writeByChapter.get(s.chapterNumber - 1);
            if (prevWrite)
                deps = [prevWrite.id];
            ...
            const own = writeByChapter.get(s.chapterNumber);
            if (own)
                deps = [own.id];
        }
 ...
 */
export function buildNovelPipelineSteps(id, title, description, config = {}) {
    ...
    const steps = [];
    let stepNum = 0;
    const addStep = (label, phase, taskType, prompt, opts = {}) => {
        stepNum++;
        steps.push({
            id: `${id}-step-${stepNum}`,
            label,
 ...
 */
export function buildBookProductionSteps(id, title, description, config = {}) {
    const chapters = Math.min(Math.max(config.targetChapters || 25, 1), 200);
    const wordsPerChapter = Math.max(config.targetWordsPerChapter || 3000, 100);
    const steps = [];
    for (let ch = 1; ch <= chapters; ch++) {
